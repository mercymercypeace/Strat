...
local MainLink = LoadLocal and "" or "https://raw.githubusercontent.com/mercymercypeace/Strat/main/ui/"
...
local Patcher = loadstring(game:HttpGet(MainLink.."TDSTools/ConvertFunc.lua", true))()

function ParametersPatch(FuncsName,...)
	if type(...) == "table" and #{...} == 1 then
		return ...
	end
	local GetFuncName = FuncsName
...
function TimeWaveWait(Wave,Min,Sec,InWave,Debug)
	...
	until tonumber(GameWave.Text) == Wave and CheckTimer(InWave)
...
if CheckPlace() then
	...
	local RSHealthCurrent = ReplicatedStorage:WaitForChild("State"):WaitForChild("Health"):WaitForChild("Current")
	local RSHealthMax = ReplicatedStorage:WaitForChild("State"):WaitForChild("Health"):WaitForChild("Max")
	...
	if VoteGUI:WaitForChild("prompt").Text == "Ready?" then
		task.spawn(function()
	...
	StratXLibrary.VoteState = VoteGUI:GetPropertyChangedSignal("Position"):Connect(function()
		...
		if currentPrompt == "Ready?" or currentPrompt == "Skip Cutscene?" then
			task.wait(2)
			RemoteFunction:InvokeServer("Voting", "Skip")
	...
	task.spawn(function()
			...
			while true do
				for i,v in next, Workspace.NPCs:GetChildren() do
					if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("HumanoidRootPart").CFrame.Y > -5 then
						if not v.RootPointer.Value.Parent then
							v:Destroy()
							continue
						end
		...
		function CheckReward()
			...
			repeat task.wait() until Rewards:FindFirstChild(1) and Rewards:FindFirstChild(2)
		...
		StratXLibrary.SignalMatchEnd = MatchGui:GetPropertyChangedSignal("Visible"):Connect(function()
			...
			if UtilitiesConfig.RestartMatch and RSHealthCurrent.Value == 0 then
...
task.spawn(function()
	...
	if not StratXLibrary.Strat.ChosenID then
		...
		repeat
			...
			for i,v in ipairs(StratXLibrary.Strat) do
				local RSMap = ReplicatedStorage:WaitForChild("State"):WaitForChild("Map")
				if v.Map.Lists[#v.Map.Lists] and typeof(RSMap.Value) == "string" and v.Map.Lists[#v.Map.Lists].Map == RSMap.Value and not StratXLibrary.Strat.ChosenID then
					StratXLibrary.Strat.ChosenID = i
					break
				end
			end
